...
#### CHG-20260915-054：修复OTA运营经理无法保存热销房型配置

- 日期：2026-09-15。
- 状态：Unreleased / 开发与生产验收中。
- 根因：页面允许`OTA_OPERATION_MANAGER`维护热销房型、渠道对应和收益目标，服务端门店写入总门禁仍仅允许平台管理员，保存时返回`REVIEW_ACCOUNT_SCOPE_FORBIDDEN`。
- 修复：仅对已授权门店放行热销房型、房型对应和收益目标读写；普通门店角色、跨门店访问、采集来源及其他管理员配置继续关闭式拒绝。
- 验证：新增角色与门店范围回归，覆盖允许操作、普通角色403、跨店404、非法目标路径403以及数据源配置403。

#### CHG-20260915-053：经营分析长期留存与异机备份

- 日期：2026-09-15。
- 状态：Unreleased / 开发与本地回归中。
- 小时事实保留 48 个月、日终事实 2 年，月/季/半年/年度汇总保留 5 年。
- 原始响应先脱敏、再用独立 AES-256-GCM 密钥加密，30–90 天自动清理。
- 新增幂等 PostgreSQL 导入、环比/同比视图以及 30 日/12 月/3 年分层异机备份。

#### CHG-20260912-052：P1管理员私聊覆盖全部门店

- 日期：2026-09-12。
- 状态：Unreleased / 开发与生产验收中。
- 将P1管理员私聊从门店群播报开关中解耦：门店未配置群Webhook或未启用群播报时，只要企微长连接在线且存在全局或门店级修复管理员绑定，仍可接收P1私聊。
- 群机器人仍严格要求门店播报已启用且Webhook已配置；不会因本次全门店覆盖而扩大群发范围。满房不触发和双渠道独立幂等规则保持不变。

#### CHG-20260912-051：P1管理员私聊接收测试

- 日期：2026-09-12。
- 状态：Unreleased / 开发与生产验收中。
- 新增门店级P1管理员私聊测试入口：必须提交真实发送确认并匹配当前机器人指纹，仅向该门店已绑定修复管理员发送醒目标注的测试消息。
- 测试链路不采集PMS/OTA、不发送企业微信群、不更新正式P1风险状态；响应只返回收件人数和送达计数，不暴露企微用户标识。
- 本次011验收另提供服务器回环专用、固定门店和固定幂等键的维护触发器；外部请求统一隐藏为404，重复调用不会再次发送。

#### CHG-20260912-050：满房停止P1预警并私聊已绑定修复管理员

- 日期：2026-09-12。
- 状态：Unreleased / 开发与生产验收中。
- 满房边界：远期入住日出租率达到或超过100%时不再生成P1候选；候选选择和消息生成同时执行上限保护，避免手工或测试入口绕过。
- 管理员私聊：未满房的有效P1继续按现有群机器人策略发送，并通过企微长连接私聊当前门店已绑定的修复管理员；两个渠道使用独立幂等消息键并行投递，任一渠道部分或全部送达后才推进P1状态。
- 安全与去重：只使用既有门店范围管理员绑定，不扩大收件人；失败渠道保留独立审计记录，不以重试另一个渠道的方式造成重复消息。

#### CHG-20260912-049：修复罗盘重新登录后的配置死锁与误导性本机登录流程

- 日期：2026-09-12。
- 状态：Unreleased / 开发与生产验收中。
- 011复现结论：罗盘会话失效后，旧的手工验证失败会把配置停用，快速修复又要求配置已启用，导致无法自助恢复；同时普通浏览器打开官网产生的登录不会进入服务器隔离会话。
- 修复：保留历史单门店指纹的失效会话即使已停用也可进入验证码快速修复；重新验证成功后自动恢复启用、采集与简报。仅从未完成单店验证的配置仍保持关闭。
- 交互：门店修复页移除无效的“登录完成，开始验证”，明确普通浏览器登录不会同步，并统一引导使用企微或后台验证码的一键快速修复。

#### CHG-20260912-048：新增罗盘双通道验证码快速修复与015企微协同恢复

- 日期：2026-09-12。
- 状态：Unreleased / 开发与生产验收中。
- 罗盘后台快速修复：门店修复页可一键复用既有的单店验证码挑战；验证码图片会私聊该门店已绑定的企业管理员，同时在登录后的后台显示，任一渠道提交都会进入同一个最多3次、10分钟有效的验证会话。
- 一致性与安全：后台提交只接受当前门店活动挑战，不接收客户端提供的挑战哈希；重复点击复用进行中的挑战，过期挑战会先关闭再创建；状态接口不返回PMS账号、密码、Cookie或修复令牌。
- 015快速恢复：驿联云继续优先使用服务端加密凭据一键自动重登并完成三接口影子校验；如厂家要求短信、滑块或手机确认，系统立即私聊本店企业管理员发送真实处理入口，不生成无法被厂家验证的伪验证码。完成厂家确认后可在后台再次一键恢复。

#### CHG-20260911-047：修复门店热销房型售罄预警的企业微信投递链路

- 日期：2026-09-11。
- 状态：Unreleased / 生产修复已发布并继续完成015兼容恢复。
- 根因：售罄预警模板按既定规则生成`mentioned_list:["@all"]`，公共企微发送器却只允许空提醒名单，导致消息在发起HTTP请求前以`WECOM_PAYLOAD_INVALID`失败。
- 修复：仅当消息正文和受信任的投递类型同时匹配时，才对热销房型售罄预警和经营综合简报两个登记模板放行精确的`@all`；其他模板继续禁止全员提醒，并新增真实生成器到公共发送器的假 HTTP 合同测试。
- 安全恢复：调度器继续对任何已有消息键禁止直接重发；只有状态明确为`REJECTED`、完整零送达、错误为本地`WECOM_PAYLOAD_INVALID`且顶层和全部分段都明确记录“未触网”的12小时内售罄预警，才会在当前小时两类简报均已送达至少1分钟、重新采集后仍可靠售罄时自动补偿一次。对旧版未记录触网状态的失败，只在正文、哈希、时间、消息键、单分段状态及无重试谱系全部严格命中历史缺陷指纹时补写带版本和时间的“请求前失败”审计证据；任何近似或矛盾记录继续只允许人工核对。`SENDING`、`AMBIGUOUS`、部分送达、损坏记录和重试子记录一律转人工核对。自动和人工重试均使用进程内合并、跨进程原子占位、持久化预检与固定操作谱系，覆盖双实例并发及服务崩溃恢复。
- 可观测性：发送记录返回并展示安全原因码、HTTP状态、企微错误码、是否已发起网络请求及分段送达数；bundle不再覆盖首个具体失败原因；服务重启时遗留的`SENDING`记录转为结果待确认，避免误重发。
- 账本门禁：企业微信投递账本格式损坏、读取失败、原子落盘失败，或已存在企微配置但账本文件丢失时一律Fail Closed，暂停群机器人和修复助手全部外发；健康检查返回`DEGRADED`和具体阻断原因，发送记录接口返回503，必须恢复账本并重启后才能重新发送。首次运行会先创建明确的空账本；所有外发入口在联网前统一取得按消息键持久化的跨进程占位，定时器和人工重试即使来自不同服务进程也只有一个能够进入网络发送。
- 发布完整性：生产打包清单加入投递状态与售罄自动及人工重试模块；未执行真实企微发送。
- 测试安全：后台测试入口改称“安全模板测试”并增加发送确认；其数据采集不写入正式快照池、不刷新OTA且不触发自动修复，P1测试正文带醒目标识，售罄预警和经营综合简报等`@all`模板明确排除；两个遗留真实发送测试API已禁用，经营综合测试脚本改为纯预览并移出生产发布包。
- 门店读取恢复：启动时仅对明确处于缺失三接口故障、旧版首次激活或中断迁移状态的单个驿联云门店补齐固定三接口；015的迁移不会顺带创建或覆盖其他门店配置。损坏的来源或修复状态文件保持原样并阻止启动，部署校验器只允许与运行时代码完全一致的逐门店迁移。
- 015历史兼容：旧版曾把驿联云业务码202（会话失效）持久化为通用的`YILIAN_REPORT_CODE_REJECTED`，使该历史状态被误判为不可自动恢复。该旧错误码现仅进入既有的30分钟限频重登录路径；新响应仍由采集器精确分类为`YILIAN_SESSION_REAUTH_REQUIRED`，登录失败、验证码和风控继续转人工处理，影子三接口全部成功前不替换旧令牌、不发布快照也不触发播报。
- 015快照恢复：三接口影子校验全部通过、令牌和营业日配置安全落盘后，将同一份已验证完整快照原子写入正式快照库，再标记恢复成功；落盘失败保留已验证的新会话并进入限频重试，且整个恢复路径不调用企业微信。调度器同时识别显式持久化的`MANUAL_REPAIR`待处理状态，使夜间人工恢复无需伪造采集错误即可立即执行。
- 015二次登录修复：对齐驿联云当前官网流程，首次`userLogin`后必须继续精确选择门店、酒店部门和当前班次，只有`confirmLogin`返回与临时令牌不同的最终令牌才可进入三接口影子校验；多门店或多酒店部门无法唯一匹配时Fail Closed。实时房态同步迁移至厂家当前`/reportAPP/nowRoomStateReport`读接口，三个报表请求统一携带官网当前的`Content-Type: text/xml`。旧实时接口在启动时逐门店定向迁移，不扩大到其他PMS配置。
- 发布保护：发布前同时执行Web与企微测试集，脏工作树、未提交运行文件、未固定SSH主机密钥均阻止发布；服务切换、配置和健康检查任一步失败都会恢复代码与受保护运行态，只有发布或回滚完整成功后才恢复调度。
...
- 修复驿联云订单源固定单页 1 条导致 015 门店触发分页上限的问题：正式契约改为官网支持的 100 条，并在启动时安全迁移旧配置。
- 兼容驿联云当前实时房态“报表头 + 房型明细列表”的两层响应结构，避免 015 门店把报表头误判为房型数据。
- 驿联云订单分页期间总数变化时会整批有界重读一次；连续不稳定仍失败关闭，避免营业中的新增或取消订单被误判为永久读取故障。
- 驿联云明确的人工修复请求不再被夜间自动任务延迟；普通定时恢复仍延后到 07:30，连续分页不稳定则按 30 分钟退避自动重试。
