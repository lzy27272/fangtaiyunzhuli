...
- 例行消息的 `mentioned_list` 固定为空；只有正文和受信任投递类型同时命中登记规则的热销房型售罄预警等模板，才允许精确的 `mentioned_list:["@all"]`。
- 后台“安全模板测试”只发送带醒目测试标识且不提醒全员的模板；它使用不落正式快照、不刷新OTA、不触发自动修复的隔离采集。热销房型售罄预警、经营综合简报等 `@all` 模板刻意排除，告警链路只通过注入假 HTTP 的合同测试验证。
- 遗留测试发送API已禁用；经营综合测试工具只输出不含正文和密钥的预览元数据，且不进入生产发布包。
- 已配置过企业微信的运行目录一旦丢失投递账本，或账本无法读取、安全落盘时，所有企业微信外发均停止，健康检查标记降级；每个正式消息键在联网前还会取得持久化跨进程原子占位，定时任务与人工重试不能重复发送同一批次。人工售罄重试另使用带原子接管守卫的任务占位，结果不明确时不得再次发送。
