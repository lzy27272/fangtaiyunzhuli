...
- Web 和 API 仍只监听服务器的 `127.0.0.1:5180/8091`，公网不直接暴露运行端口。
- Docker Compose 不发布任何公网端口。
- 第一阶段公网入口只由既有 HTTPS Caddy 反向代理受控路径，SSH 隧道保留为
  运维备用入口。
...
公网只开放标准 HTTPS 443；`/ota-console/` 提供页面，
`/api/v1/ota-console/` 转发到回环 API。服务器上的 5180/8091 不对公网监听。
后台部署在云服务器上，因此部署人员的本地电脑关闭不会影响门店访问。

平台管理员登录后，在“人员与权限”中为每位管理人员创建独立账号并勾选可访问
门店。服务端会再次校验门店范围，不能只依赖页面隐藏：非管理员只能看到授权门店，
也不能读取或编辑采集配置，但可以在授权门店内使用 PMS/OTA 登录修复功能。
管理员停用账号、修改密码或调整权限后，原登录会话立即失效。

短期访问令牌有效 10 分钟，仅保存在页面内存；浏览器使用 HttpOnly 刷新会话恢复
登录，最长 30 天且每次刷新都会轮换。刷新会话只以哈希保存在服务器，退出、密码
变更和账号停用都会撤销会话。连续登录失败会触发临时限流，认证及越权操作写入
不含密码、Cookie、验证码或令牌的安全审计记录。
