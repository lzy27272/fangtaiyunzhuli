  ...
  if (incidentResult.status === 'fulfilled') {
    for (const incident of incidentResult.value.filter((item) => !/CLOSED|RESOLVED/i.test(item.status))) {
      ...
      issues.push({
        ...
        detail: safeBusinessText(incident.type, KIND_LABEL[kind]),
...
export function ExceptionCenterPage({
  ...
  const safeAction = async () => {
    ...
    if (issue.kind === 'LOGIN' || issue.kind === 'OFFLINE') {
      setSelected(null); onOpenStore(issue.hotel, 'collection'); return
    }
