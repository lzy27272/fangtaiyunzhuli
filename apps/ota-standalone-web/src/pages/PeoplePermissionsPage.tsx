...
const ROLE_OPTIONS = Object.entries(ROLE_LABELS).filter(([code]) => code !== 'PLATFORM_ADMIN') as Array<[OtaRole, string]>
const ROLE_TEMPLATES: Array<{ role: OtaRole; detail: string; permissions: string[] }> = [
  { role: 'OTA_OPERATION_ASSISTANT', detail: '查看门店状态与播报记录', permissions: ['查看总览', '查看门店数据', '查看播报记录'] },
  { role: 'OTA_OPERATION_MANAGER', detail: '管理采集连接并处理异常', permissions: ['查看总览', '采集配置', '异常处理', '播报检查'] },
  { role: 'REVENUE_MANAGER', detail: '维护热销房型和销售目标', permissions: ['查看经营数据', '运营配置', '销售目标'] },
  { role: 'REGIONAL_MANAGER', detail: '查看授权区域的全部门店', permissions: ['区域总览', '经营数据', '异常记录'] },
  { role: 'HOTEL_P1_HANDLER', detail: '处理指定门店高优先级异常', permissions: ['异常中心', '重新采集', '登录修复入口'] },
]
...
const emptyDraft: AccountDraft = { username: '', displayName: '', password: '', roles: ['OTA_OPERATION_MANAGER'], hotelIds: [], enabled: true }
...
export function PeoplePermissionsPage({ session, hotels }: { session: AuthSession; hotels: SimulationHotelView[] }) {
  ...
  function openEdit(account: ManagedAccount) {
    setEditing(account)
    setDraft({ username: account.username, displayName: account.displayName, password: '', roles: account.roles, hotelIds: account.hotelIds ?? [], enabled: account.enabled })
  ...
  return (
    <section className="console-page people-page">
      ...
      <div className="summary-strip"><div><span>全部账号</span><strong>{accounts.length}</strong><small>含平台管理员</small></div><div><span>启用账号</span><strong>{enabledCount}</strong><small>可正常登录</small></div><div><span>平台管理员</span><strong>{platformAdmins}</strong><small>拥有全部门店权限</small></div></div>
