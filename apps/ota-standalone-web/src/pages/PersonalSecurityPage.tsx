...
export function PersonalSecurityPage({ session, onSessionChange }: { session: AuthSession; onSessionChange: (session: AuthSession) => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    } catch (cause) { setError(cause instanceof Error ? cause.message : '账号安全设置保存失败') }
    finally { setSubmitting(false) }
  }

  return <section className="console-page personal-security-page">
    <div className="page-title-row"><div><p className="section-kicker">ACCOUNT SECURITY</p><h1>账号安全</h1><p>修改当前账号的登录资料，不影响业务平台登录和 Cookie。</p></div><Status tone="ok"><Icon name="shield" size={14} />会话受保护</Status></div>
    <div className="security-layout">
      <section className="content-panel account-profile"><span className="large-avatar">{session.account.displayName.slice(0, 1)}</span><div><h2>{session.account.displayName}</h2><p>{session.username}</p></div><dl><div><dt>账号角色</dt><dd>{session.account.roles.map((role) => ROLE_LABELS[role]).join('、')}</dd></div><div><dt>门店范围</dt><dd>{session.account.hotelIds === null ? '全部门店' : `${session.account.hotelIds.length} 家已授权门店`}</dd></div><div><dt>登录方式</dt><dd>本地账号 + 安全 Cookie</dd></div></dl></section>
