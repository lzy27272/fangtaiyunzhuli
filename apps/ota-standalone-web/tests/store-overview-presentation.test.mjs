...
test('store detail renders every configured OTA data result instead of status only', async () => {
  ...
  assert.match(otaPanel, /function OrderDatasetBoard/u)
  assert.match(otaPanel, /平台返回总量/u)
  assert.match(otaPanel, /分页完整性/u)
  assert.match(otaPanel, /不展示订单号、住客、房型、联系方式或任何订单明细/u)
  assert.match(otaPanel, /重新登录并更新Cookie/u)
